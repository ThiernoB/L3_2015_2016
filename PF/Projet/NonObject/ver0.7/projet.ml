...
let string_of_dimacs (in_chan : in_channel) : string =
  stripchars (input_line in_chan) " "


let create_dimacs (form, nb_v, nb_l : formula * int * int) (out_chan : out_channel) : unit =
  output_string out_chan("p cnf "^(string_of_int nb_v)^" ");
  output_string out_chan (string_of_int (nb_l * nb_v)^"\n");
  output_string out_chan ((dimacs_of_formula form) ^ "0");
  ()

let exec_minisat (file_name : string) : int =
  try
    (Sys.command ("minisat "^file_name^" sortie.txt"))
  with _ -> 2
...
let show_stables () : unit =
  ...
  create_dimacs (form, (d * d), List.length auto.rules) out_chan;

  (* exec_minisat "sortie"; 
  let rec t () =
    
    if exec_minisat "sortie" != 0 then
      exit 2
    else
      begin
	let in_chan = open_in "sortie" in
	output_string out_chan (string_of_dimacs (open_in "sortie"));
	close_in in_chan;
      end;
    
    let h =  match read_line() with
