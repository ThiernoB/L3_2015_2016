...
let verif_rule (gene : generation) (n : int) (i : int) (j : int) (v : state list) : bool =
  ...
  (state_compare state1 v1) && (state_compare state2 v2) && (state_compare state3 v3) && (state_compare state4 v4)
...
let stables (auto, d : automaton * int) : formula  =  
  ...
  formula_of_list_formula !rules
	
let rec formula_to_dimacs (form : formula) : string = match form with
    True | False -> ""
  | Var s -> string_of_int s^" "
  | Neg f -> "-"^(formula_to_dimacs f)
  | Or(f1, f2) -> (formula_to_dimacs f1)^(formula_to_dimacs f2)
  | And(f1, f2) ->
    begin match f1 with
      | True | False -> (formula_to_dimacs f2)
      | _ -> 
	match f2 with
	  | True | False -> (formula_to_dimacs f1)
	  | _ -> (formula_to_dimacs f1) ^ "0\n" ^ (formula_to_dimacs f2)
    end

let show_stables () : unit =
  let d, auto, g = parse (open_in "autretest.txt") in
  let form = stables (auto, d) in
  let out_chan = open_out "entree.dimacs" in
  let n = nb_clauses form in
  output_string out_chan "p cnf ";
  output_string out_chan ((string_of_int (d * d))^" ");
  output_string out_chan (string_of_int n);
  output_string out_chan "\n";
  output_string out_chan ((formula_to_dimacs form) ^ "0");
  close_out out_chan;
  ()
	
let exec_minisat (file_name : string) : int =
  exit (Sys.command ("minisat "^file_name^" sortie.txt"))
    
(* TESTS
let (d, auto, gene) = parse (open_in "autretest.txt");;
