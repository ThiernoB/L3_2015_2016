...
(*
  let rec global_loop () =
    match exec_minisat () with
	2 -> close_in in_chan; exit 2
      | _ -> 
	try
	  let rec local_loop () = match (input_line in_chan) with
	    | "SAT" -> local_loop ();
	    | "UNSAT" -> Printf.printf "UNSAT\n";
	    | str -> let s = string_of_int_list (int_list_of_minisat (stripchars str " ")) in
		     if String.length str > 0 then
		       begin
			 nb_l := !nb_l + 1;
			 output_string out_chan s;
			 incr_lines out_chan nb_v !nb_l;
			 global_loop()
		       end
	  in local_loop ();
	with End_of_file -> ();
