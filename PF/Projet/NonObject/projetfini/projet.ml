...
**	@email : rgv26.warforce@hotmail.fr - yazid_lefoot@hotmail.fr
**
*)
type formula =
    True | False
...
let minisleep (sec : float) : unit =
  try
    ignore (Unix.select [] [] [] sec)
  with _ -> ()
;;

let show_time (fic : string) (time:float) (nbgen:int)=
  Graphics.open_graph " 500x500";
  let (d, auto, gene) = parse (open_in fic) in
  let nb = ref 0 in
  let g = ref gene in
...
let int_list_of_minisat (str : string) : int list =
 List.map (function x->x* (-1))(List.map int_of_string (Str.split (Str.regexp(" ")) str));
;;
...
let show_stables (file : string) : unit =
  let d, auto, g = parse (open_in file) in
	...
	let exec_command () = match (Sys.command ("minisat entree.dimacs sortie")) with
	  ...
	  | _ ->
	    let rec local_loop in_chan = match (input_line in_chan) with
	      ...
	      | str -> 
		let l = int_list_of_minisat str in
...
let show () =
  Graphics.open_graph " 500x500";
  Graphics.set_window_title "salut"
;;

let main () : unit =
  try
    let automate_file = Sys.argv.(1) in   
    let time = float_of_string (Sys.argv.(2)) in
    let nb = int_of_string (Sys.argv.(3)) in 
    show_stables automate_file;
    show_time automate_file time nb;
  with _ -> ()
in main ()
;;
